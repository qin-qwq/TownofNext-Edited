...
// Reference: https://github.com/Gurge44/EndlessHostRoles/blob/main/Modules/Extensions/CountdownTimer.cs
public sealed class CountdownTimer : IDisposable
{
    ...
    public void Start()
    {
        ...
        _completed = false;
        Stopwatch.Restart();
        _coroutine = Main.Instance.StartCoroutineV2(_hasTickEvent ? Run() : RunWithoutTicks());
    }
    ...
    {
        if (_cancelOnMeeting && (GameStates.IsMeeting || ExileController.Instance || AntiBlackout.SkipTasks)) return true;
    ...
    public void Dispose()
    {
        if (_coroutine != null)
        {
            Main.Instance.StopCoroutineV2(_coroutine);
            _coroutine = null;
        }
