...
public class PlayerGameOptionsSender(PlayerControl player) : GameOptionsSender
{
    ...
    public override IGameOptions BasedGameOptions => GameStates.IsNormalGame ?
            Main.RealOptionsData.Restore(new NormalGameOptionsV09(new UnityLogger().CastFast<ILogger>()).CastFast<IGameOptions>()) : Main.RealOptionsData.Restore(new HideNSeekGameOptionsV09(new UnityLogger().CastFast<ILogger>()).CastFast<IGameOptions>());
