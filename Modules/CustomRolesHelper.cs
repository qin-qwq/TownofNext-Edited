...
using static TONE.Roles.Core.CustomRoleManager;
...
public static class CustomRolesHelper
{
    ...
    public static CustomRoles GetVNRole(this CustomRoles role) // RoleBase: Impostor, Shapeshifter, Crewmate, Engineer, Scientist
    {
        ...
        return role switch
        {
            CustomRoles.ShapeshifterTONE => CustomRoles.Shapeshifter,
            CustomRoles.PhantomTONE => CustomRoles.Phantom,
            CustomRoles.ScientistTONE => CustomRoles.Scientist,
            CustomRoles.EngineerTONE => CustomRoles.Engineer,
            CustomRoles.NoisemakerTONE => CustomRoles.Noisemaker,
    ...
    // Role's basis role is an Impostor (regular imp,shapeshifter,phantom,viper) role
    ...
    */
    ...
    {
        return target.Is(CustomRoles.EngineerTONE)
    ...
    public static bool IsSpeedRole(this CustomRoles role)
    {
        return role is
            ...
            CustomRoles.Dreamer or
            CustomRoles.Drunkard or
            CustomRoles.Mini;
    }
    ...
    public static bool CanBeRecruitedBy(this PlayerControl target, PlayerControl recruiter)
    {
        ...
        if (target.GetCustomRole() is CustomRoles.Mini && Mini.Age < 18) return false;
        ...
        switch (role)
        {
            ...
            case CustomRoles.Autopsy:
                if (pc.Is(CustomRoles.Doctor)
                    ...
                    || pc.Is(CustomRoles.ScientistTONE)
            ...
            case CustomRoles.Tiebreaker:
                if (pc.Is(CustomRoles.Dictator)
                    ...
                    || pc.Is(CustomRoles.Influenced)
                    || pc.Is(CustomRoles.GuardianAngelTONE)) return false;
                break;
            ...
            case CustomRoles.Nimble:
                if (Vigilante.CheckCanUseVent(pc)
                    || pc.Is(CustomRoles.CopyCat))
                    return false;
            ...
            case CustomRoles.Flash:
                if (pc.Is(CustomRoles.Swooper)
                    ...
                    || pc.Is(CustomRoles.ShapeshifterTONE)
            ...
            case CustomRoles.Mini:
                if (pc.Is(CustomRoles.DollMaster)
                    || pc.Is(CustomRoles.Doppelganger)
                    || pc.Is(CustomRoles.ShapeMaster)
                    || pc.Is(CustomRoles.ShapeshifterTONE)
                    || pc.Is(CustomRoles.Morphling)
                    ...
                    || pc.Is(CustomRoles.Retributionist)
                    || pc.Is(CustomRoles.Dictator)
                    || pc.Is(CustomRoles.Revenant)
                    || pc.Is(CustomRoles.SchrodingersCat)
                    || pc.Is(CustomRoles.Innocent)
                    || pc.Is(CustomRoles.PunchingBag)
                    || pc.Is(CustomRoles.Specter)
                    || pc.Is(CustomRoles.Terrorist)
                    || pc.Is(CustomRoles.Glitch)
                    || pc.Is(CustomRoles.Traitor)
                    || pc.Is(CustomRoles.Veteran)
                    || pc.Is(CustomRoles.Sheriff)
