...
public static class Utils
{
    ...
    public static string GetAbilityTimeDisplay(PlayerControl seer, PlayerControl seen)
    {
        try
        {
    ...
    public static void SendMessage(string text, byte sendTo = byte.MaxValue, string title = "", bool logforChatManager = false, bool noReplay = false, bool ShouldSplit = false, SendOption sendOption = SendOption.Reliable)
    {
        if (!AmongUsClient.Instance.AmHost) return;
    ...
    public static void ApplySuffix(PlayerControl player)
    {
        ...
        {
            name = player.IsModded() ? "<size=1.2>" + player.FriendCode.GetDevUser().GetTag() + "</size>" + $"\r\r\n" + "<size=1.5>" + modtag + "</size>" + name
                : "<size=1.5>" + $"{player.FriendCode.GetDevUser().GetTag()}" + "</size>" + " " + "<size=1.5>" + modtag + "</size>" + name;
        }
    ...
    public static bool DeathReasonIsEnable(this PlayerState.DeathReason reason, bool checkbanned = false)
    {
        ...
        return checkbanned ? !BannedReason(reason) : reason switch
        {
            ...
            PlayerState.DeathReason.Torched => (CustomRoles.Arsonist.IsEnable() || CustomRoles.Pyrophoric.IsEnable()),
            ...
            PlayerState.DeathReason.Drained => CustomRoles.Puppeteer.IsEnable() || CustomRoles.Dreamer.IsEnable(),
