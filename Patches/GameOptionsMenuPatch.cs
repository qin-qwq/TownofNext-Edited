    ...
    private static bool InitializePrefix(StringOption __instance)
    {
        if (ModGameOptionsMenu.OptionList.TryGetValue(__instance.GetInstanceID(), out var index))
        {
            ...
            if (EnumHelper.GetAllValues<CustomRoles>().Find(x => GetString($"{x}") == name1.RemoveHtmlTags(), out var role))
            {
                ...
                if (role.NotAssignInVanillaServer() || (Options.CurrentGameMode == CustomGameMode.RoundUp && role.NotSpawnInRoundUp())) name += GetString("NotSupportsVanilla");
    ...
    private static bool UpdateValuePrefix(StringOption __instance)
    {
        if (ModGameOptionsMenu.OptionList.TryGetValue(__instance.GetInstanceID(), out var index))
        {
            ...
            if (item is PresetOptionItem || (item is StringOptionItem && item.Name == "GameMode"))
            {
                ...
                var other = hns - 1;
                if (Options.GameMode.GetInt() == hns && !GameStates.IsHideNSeek) //Hide And Seek
                {
                    if (Options.prevGameMode == other) Options.GameMode.SetValue(0);
                    else Options.GameMode.SetValue(other);
                }
                else if (Options.GameMode.GetInt() != hns && GameStates.IsHideNSeek)
                {
